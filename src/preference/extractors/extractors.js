...
/* eslint-disable camelcase, valid-jsdoc */
...
/*
  ...
  returns [0,n]
*/
export function encodingLevel(record) {
...
export function fenniOrNotLDR(record) {
	if (record.leader === undefined || record.leader.length < 17) {
		return undefined;
	}
...
/*
  ...
  returns [0,1,2,3,4]
*/
export function catalogingSourceFrom008(record) {
...
export function fenniOrNotFrom008(record) {
	const extractFunc = controlfieldPosition('008', 39);
	const value = extractFunc(record);
	return value === '^' ? 1 : 0;
}
...
*/
export function nonFinnishHELKA(record) {
...
export function specificSingleLocalOwner(localOwnerTag) {
...
*/
export function recordAge(record) {
	const extractFunc = controlfieldPosition('008', 0, 6);
	return extractFunc(record);
}

export function reprintInfo(record) {
...
/*
  ...
  returns [0,n]
*/
export function localOwnerCount(record) {
...
export function fieldCount(tag, arrayOfSubfields) {
	return function (record) {
...
export function subfieldCount(tag) {
	return function (record) {
		const fieldsWithTag = record.fields.filter(field => field.tag === tag);
...
export function uppercaseSubfield(record) {
...
export function specificFieldValue(tag, arrayOfSubfields, lookupValues) {
	return function (record) {
...
export function specificField(tag, subfieldList) {
	return function (record) {
		return _.chain(record.fields)
			.filter(field => field.tag === tag)
			.flatMap(field => field.subfields)
...
/*
  ...
  returns [0,n]
*/
export function fieldLength(tag) {
	return function (record) {
		return _.chain(record.fields)
			.filter(field => field.tag === tag)
			.flatMap(field => field.subfields)
...
export function specificLocalOwner(localOwnerTag) {
...
export function latestChange(nameFilterFunction) {
...
export function parseIntegerProperty(obj, propName) {
	return (obj[propName] === undefined) ? 0 : parseInt(obj[propName], 10);
}

export function field008nonEmptyCount(record) {
...
export function containsValue(tags, values) {
	return function (record) {
		return _.chain(record.fields)
