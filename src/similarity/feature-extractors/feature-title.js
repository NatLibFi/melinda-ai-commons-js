...
import {bibTitleStopWords as stopWordData} from '../../config/feature-extractors-title';
...
const stopWords = stopWordData.split('\n')
	...
	.map(line => line.split(' '))
	.filter(pair => parseInt(_.head(pair), 10) > STOP_WORD_FREQUENCY)
...
export default function (record1, record2) {
	...
	function check() {
		...
		if (compareFuncs.isSubset(set1, set2) || compareFuncs.isSubset(set2, set1)) {
			const subs1 = set1.reduce((memo, field) => memo.concat(field.subfield), []);
			const subs2 = set2.reduce((memo, field) => memo.concat(field.subfield), []);
